<?php
/**
 * Plugin Name: MX POS Payment Method Correction
 * Description: Corrección operativa para actualizar método de pago POS cash/card y sincronizar caja.
 * Version: 0.1.0
 */
...
add_action('rest_api_init', function (): void {
    register_rest_route('mx-pos/v1', '/sales/(?P<id>\d+)/payment-method', [
        ...
        'callback'            => 'mx_pos_payment_correction_update_sale_payment_method',
        'permission_callback' => 'mx_pos_payment_correction_payment_method_permission',
        'args'                => [
            'id' => [
                'required'          => true,
...
function mx_pos_payment_correction_payment_method_permission(WP_REST_Request $request): bool
{
    return is_user_logged_in()
        && (
            current_user_can('manage_options')
            || current_user_can('manage_woocommerce')
            || current_user_can('mx_pos_sell')
            || current_user_can('mx_pos_refund')
            || current_user_can('mx_pos_view_reports')
        );
}

function mx_pos_payment_correction_update_sale_payment_method(WP_REST_Request $request): WP_REST_Response|WP_Error
{
    global $wpdb;
    ...
    $orderTotal     = mx_pos_payment_correction_format_decimal((float) $order->get_total());
    ...
    try {
        ...
        $summary = mx_pos_payment_correction_decode_json_array($sale['payment_summary'] ?? null);
        ...
        $cashAction = mx_pos_payment_correction_sync_cash_movement(
            $cashMovementsTable,
            $sale,
            $order,
            $clientRequestId,
        ...
        mx_pos_payment_correction_log_audit($sale, $orderId, $oldSlug, $oldTitle, $newSlug, $newTitle, $cashAction);
...
function mx_pos_payment_correction_sync_cash_movement(
...
function mx_pos_payment_correction_log_audit(
    array $sale,
    int $orderId,
    string $oldSlug,
    string $oldTitle,
...
function mx_pos_payment_correction_decode_json_array(mixed $value): array
{
...
function mx_pos_payment_correction_format_decimal(float $value): string
{
