...
/**
 * Admin form handlers in this file are restricted to authorized WordPress admins.
 * Some POST branches are validated centrally by the handler before persistence, while
 * Plugin Check reports each individual field access.
 *
 * Password fields are intentionally not sanitized with text sanitizers because doing so
 * could alter valid password characters before hashing/verification.
 *
 * rootlabs-pos-pro-w3-admin-form-fields
 *
...
class AdminPage
{
    ...
    {
        add_menu_page(
            esc_html__('RootLabs POS', 'mx-pos-pro'),
            esc_html__('RootLabs POS', 'mx-pos-pro'),
            'manage_options',
            self::MENU_SLUG,
            [$this, 'render'],
    ...
    public function render(): void
    {
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('RootLabs POS', 'mx-pos-pro'); ?></h1>
    ...
    private function render_estado(): void
    {
        ...
        if ($db_version !== $expected_db) {
            $warnings[] = sprintf(
                ...
                __('La versión de base de datos (%1$s) no coincide con la esperada (%2$s).', 'mx-pos-pro'),
                $db_version ?: '—',
                $expected_db
            );
        }
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Status HTML is generated internally and limited to admin UI markup.
                            ...
                            <td>
                                <select id="mx_pos_register_branch" name="register_branch_id" required>
                                    ...
                                    <?php foreach ($branches as $b):
                                        ...
                                        ?>
                                        <option value="<?php echo esc_attr((string) $b['id']); ?>" <?php echo esc_attr( $selected ); ?>>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($edit_method): ?>
                                    ...
                                    <p class="description"><?php esc_html_e('El slug se conserva por compatibilidad histórica.', 'mx-pos-pro'); ?></p>
                                <?php else: ?>
                                    <input type="text"
                                           id="mx_pos_payment_method_slug"
            ...
            <div class="mx-pos-section">
                ...
                <table class="form-table">
                    ...
                    <tr>
                        ...
                        <td>
                            <?php
                            /* translators: %s is the masked Telegram bot token. */
                            $current_token_placeholder = $masked_token !== '' ? sprintf( __( 'Current: %s', 'mx-pos-pro' ), $masked_token ) : '';
                            /* translators: %s is the masked Telegram bot token. */
                            $current_token_description = $masked_token !== '' ? sprintf( __( 'Current token: %s', 'mx-pos-pro' ), $masked_token ) : '';
                            ?>
                            <input type="password"
                                   id="mx_pos_telegram_bot_token"
                                   name="mx_pos_telegram_bot_token"
                                   class="regular-text"
                                   placeholder="<?php echo esc_attr( $current_token_placeholder ); ?>"
                                   value=""
                                   autocomplete="off" />
                            <?php if ($masked_token !== ''): ?>
                                <p class="description">
                                    <?php echo esc_html( $current_token_description ); ?>
                                </p>
                                <p class="description">
                                    <label>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <div id="mx-pos-logo-preview" class="mx-pos-logo-preview">
                                <?php echo wp_kses_post( $logo_preview_html ); ?>
                            </div>
